...
func TestNewHandshakeV10FullPacket(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, packet.ProtocolVersion, byte(0x4a))
	assert.Equal(t, packet.ServerVersion, "\x00\x00\x00\n5.6.25")
	assert.Equal(t, packet.ConnectionId, [4]byte{0x9e, 0x2e, 0x00, 0x00})
	assert.Equal(t, packet.AuthPluginData, `Oa{eh\sNiHj]sJUPpd$%`)
...
func TestNewHandshakeV10ShortPacket(t *testing.T) {
	...
	assert.Equal(t, packet.ServerVersion, "\x00\x00\x00\n5.6.25")
	assert.Equal(t, packet.ConnectionId, [4]byte{0x9e, 0x2e, 0x00, 0x00})
	assert.Equal(t, packet.AuthPluginData, `Oa{eh\sN`)
